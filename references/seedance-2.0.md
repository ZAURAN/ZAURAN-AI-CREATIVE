...
4. **Состояние проекта секвенции** — `seedance-2.0/references/sequence-project-state.md`, `seedance-2.0/references/continuation-handoff.md`, схема `seedance-2.0/schemas/project-state.schema.json`. Для многоклиповой работы держать состояние: канон персонажей и локаций, принятые тейки, наблюдаемое конечное состояние, глубина цепочки. Продолжение писать от наблюдаемого конца, а не от задуманного. Ложится рядом с существующим `PROMPT_vN.md`: промпт версионируется в `PROMPT_vN.md`, состояние проекта — в `project-state.json` проектной папки.
