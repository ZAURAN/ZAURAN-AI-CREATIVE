...
| Extend | Continue from accepted source footage only. | Starting from a planned ending or inventing the clip state. | Route to [seedance-continuation](../seedance-continuation/GUIDE.md) and use the observed end state. |

## Sequence Boundary

The generic prompt skill must not independently invent continuation state. If the user asks to continue, extend, make part two, or use a previous clip, route to [seedance-continuation](../seedance-continuation/GUIDE.md) unless the accepted clip/final frame and observed end state are already present in the sequence state.
