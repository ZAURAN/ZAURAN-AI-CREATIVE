...
Всё живёт в папке проекта на диске (по умолчанию `C:\Users\ganba\Videos\Креативы\<Проект>\story\`; структура и правила — `references/project-vault.md`). Первое действие в любом чате: найти и прочитать `STATE.md`. Есть — выполнить **протокол продолжения** из `project-vault.md` (STATE → TIMELINE → CANON → последний SCRIPT → NOTES → сводка пользователю), и только потом творить. Нет — новый проект: создать папку и `STATE.md` сразу после STORY LOCK. Каждое утверждение и конец сессии = запись на диск. Файл главнее чата.
