...
Скилл для Claude Code (и совместимых агентов с поддержкой `SKILL.md`), который ведёт креативную AI-задачу от идеи до готового файла: бриф → выбор модели → креативное направление → раскадровка → промт → генерация → проверка → выдача → запись выводов в проектную папку.
...
7. **Learning capture.** После реальной генерации пишет в `NOTES.md` проектной папки: модель, промт/настройки, что сработало, что нет, какую правку подтвердил пользователь.
...
Внешняя база заметок (Obsidian и т. п.) не нужна — скилл самодостаточен. Личные заметки подключаются только если явно назвать папку в чате.
